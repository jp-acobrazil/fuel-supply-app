...
const totalPages = computed(() => {
  const n = Math.ceil((filtered.value?.length || 0) / pageSize)
  return n > 0 ? n : 1
})
      ...
      <div class="card">
        ...
        <div class="table-wrapper" v-if="filtered.length">
            ...
            <tbody>
              <tr v-for="s in paged" :key="s.id">
                ...
                <td class="row-actions">
                  ...
                  <teleport to="body">
                    <div v-if="actionSupplyId === s.id" class="pop pop-actions"
                      :style="{ top: popCoords.top + 'px', left: popCoords.left + 'px' }" @click.stop>
          ...
          <div class="pagination">
            <button class="page-btn" @click="goFirst" :disabled="currentPage === 1">«</button>
            ...
            <button class="page-btn" @click="goLast" :disabled="currentPage === totalPages">»</button>
          </div>
        </div>
...
.pop-actions {
  ...
  flex-direction: column;
  min-width: 140px;
  box-shadow: 0 6px 22px -4px rgba(0, 0, 0, .18), 0 4px 10px -2px rgba(0, 0, 0, .08);
  z-index: 9999;
}
